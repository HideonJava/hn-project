<template>
  <div>
    <div id="closed" :style="{ height: height, width: width }" />
  </div>
</template>

<script>
import * as echarts from 'echarts'
import { GetClosedTend } from '@/api/charts'
...
export default {
  ...
  mounted() {
    this.chart = echarts.init(document.getElementById('closed'), 'walden')
    this.$nextTick(() => {
      this.initChart()
    })
  },
      ...
      GetClosedTend(data).then((res) => {
        if (res) {
          ...
          this.chart.setOption({
            title: {
              show: true,
              text: '闭环趋势(近30天)(' + date[0] + '至' + date[date.length - 1] + ')'
            },
            tooltip: {
              trigger: 'axis',
              axisPointer: {
